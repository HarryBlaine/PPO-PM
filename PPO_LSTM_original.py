...
def main():
    ...
    while(n_epi!= 3000):
        ...
        Note = open('./DAX_data/training(original).txt', mode='w')
        ...
        if n_epi% 10 == 0:
            ...
            Note_2 = open('./DAX_data/test(original).txt', mode='w')
    ...
    plt.plot(y, 'b')
    p1 = "./image/train_image_DAX_test(original)"
    plt.savefig(p1)
    plt.figure()
    ...
    plt.plot(y, 'b')
    p2 = "./image/test_image_DAX_test(original)"
    plt.savefig(p2)
    plt.show()
    env.close()
