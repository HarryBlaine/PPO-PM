...
from env.Portfolio_management_RESNET import CustomEnv
...
def main():
    ...
    while (n_epi != 3000):
        ...
        random_train_reward.append(train_reward_list)
        ...
        if n_epi % 30 == 0:
            ...
            while (done == False):
                ...
                train_reward_list = train_reward_list * (1 + r)
                s = next_s

            total_train_reward.append(train_reward_list)

            print("# Training!!!Step: {}, total_wealth :{}".format(t, train_reward_list))
    ...
    plt_random_reward = np.array(random_train_reward)

    plt.plot(plt_random_reward, 'y')
    y = savgol_filter(plt_random_reward, 51, 3, mode='nearest')
    plt.plot(y, 'b')
    p1 = "./image/random_image_DAX_simple_rd"
    plt.savefig(p1)
    plt.figure()
